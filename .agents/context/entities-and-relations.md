...
- **Viewer**: Each Work has a dedicated full-page Viewer view at `/works/:id`. Three Viewer types: `EditViewer`, `PosterViewer`, `StoryboardViewer`, all built on `ViewerFrame`.
...
- **Format**: Users can post **Lines** (text sentences/notes), **Pin** Works/Originals with an attached Line, or post **Ledger Entries** (curated dossier logs of Originals).
...
When a user opens a Work, they enter the **Viewer** — a dedicated full-page cinematic view.
...
### `ViewerFrame` (shared wrapper)
All three work types (`EditViewer`, `PosterViewer`, `StoryboardViewer`) render inside `ViewerFrame`, which provides:
- Floating `ViewerNav` (Back + Share + Originals).
