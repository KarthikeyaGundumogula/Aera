...
## Skill Format
- Skills live in `.agents/skills/<name>.md` and `~/.agents/skills/<name>.md`.
...
14. **No Unrequested Default Values**: NEVER inject or hardcode default values into form inputs, state initializers, or API request payloads unless explicitly instructed by the user. If an optional field is left blank, omit it from the payload (`undefined` / `null`) so the backend stores `NULL` cleanly.
