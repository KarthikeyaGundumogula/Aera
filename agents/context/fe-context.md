...
## Recommendation Score — The Core Mechanic
- **Nature**: The score is a **raw, unbounded integer** — it has no maximum value and no defined scale.
- **Measurement**: It is measured relative to the recommending artist's **last personal peak score** (`lastPeakScore`). The peak is dynamic — it updates each time the user sets a new personal high.
- **Meaning**: The score is a signal of *how much this Original moved the recommender*, not a quality rating. Two recommendations of the same film can have wildly different scores from different artists.
- **Display Rule**: Scores are always shown as plain integers — **no commas, no units**. The raw number carries the weight.

## Create Recommendation — The Resonance Hold Interaction
- **Component**: `src/components/CreateRecommendationModal.tsx`
- **Trigger**: FAB → "New Recommendation" ActionNode (Sparkles icon, angleDeg: -105).
- **Single-Step Design**: The modal is one screen — Original name, Notes (optional), and the score interaction. No wizards, no multi-step flows.
- **The "Resonance Hold" mechanic**:
  - Instead of typing a number or dragging a slider (which requires a max bound), the score is generated by **holding a button**.
  - The score grows at an **exponential rate** while held: `rate = BASE_RATE * (ACCELERATION ^ secondsHeld)`. This means the first second is slow and controlled; sustained holding produces large numbers naturally.
  - **Peak Crossing UX**: When the score crosses the user's `lastPeakScore`, a radial amber flash fires, the score colour shifts from white → amber, and the peak reference gets a strikethrough — communicating that this is a rare, significant act.
  - **Spring Shake**: As the score approaches the peak (>70% of peak), the score display starts physically shaking using Framer Motion `useSpring` to build tension.
  - **Reset**: A minimal "Reset" text button appears once the score is > 0, allowing the user to start over without closing the modal.
- **Contextual Language**: The modal copy frames the act as a "good will move" — "Pass it forward. Your score is a signal, not a rating."
- **Performance**: The hold loop uses `requestAnimationFrame` and a mutable `ref` (not state) to track the score during the hold, flushing to `setState` each frame for display. This avoids stale closure issues and keeps the loop at ~60fps without re-render overhead.
