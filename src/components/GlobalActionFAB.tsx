...
const pStart = {
  x: Math.cos(sweepStart) * ARC_RADIUS,
  y: Math.sin(sweepStart) * ARC_RADIUS,
};
const pEnd = {
  x: Math.cos(sweepEnd) * ARC_RADIUS,
  y: Math.sin(sweepEnd) * ARC_RADIUS,
};
...
const oStart = {
  x: Math.cos(sweepStart) * outerR,
  y: Math.sin(sweepStart) * outerR,
};
...
const iStart = {
  x: Math.cos(sweepStart) * innerR,
  y: Math.sin(sweepStart) * innerR,
};
  ...
  useEffect(() => {
    ...
    const isAtTop = () => {
      ...
      for (let i = 0; i < containers.length; i++) {
        if (
          containers[i].clientHeight >= window.innerHeight * 0.5 &&
          containers[i].scrollTop > 10
        ) {
          return false;
        }
      }
      ...
      <AnimatePresence>
        {showHint && (
          ...
          >
            ...
            >
              <span className="text-[8px] font-black uppercase tracking-[0.3em] text-[#B45309] mb-0.5">
                Quick Actions
              </span>
              <span className="text-[11px] font-medium text-white/90 whitespace-nowrap">
                Tap to Create • Hold for Picks
              </span>
            </div>
            <button
              onClick={(e) => {
      ...
      >
        ...
        >
          ...
          >
            {isMenuOpen ? (
              <X
                className="w-6 h-6 text-[#D97706] transition-colors duration-300 pointer-events-none"
                strokeWidth={2}
              />
            ) : (
              <Clapperboard
          ...
          <motion.span
            ...
            animate={
              showHint
                ? {
                    boxShadow: [
                      "0 0 0px 0px rgba(180,83,9,0)",
                      "0 0 24px 4px rgba(180,83,9,0.25)",
                      "0 0 0px 0px rgba(180,83,9,0)",
                    ],
                  }
