...
export const SaveAction: React.FC<ActionProps> = ({ isActive, onClick, count, variant = "feed", className = "" }) => {
  ...
  return (
    <button onPointerDown={(e) => e.stopPropagation()} onClick={actionHandler} className={classes}>
      ...
      />
      {isFeed ? (
        <span className="hidden sm:inline">{isActive ? "Saved" : "Save"}</span>
      ) : (
        <span className="text-[11px] font-bold hidden sm:inline-block">
          {count ?? (isActive ? "Saved" : "Save")}
        </span>
      )}
    </button>
  );
