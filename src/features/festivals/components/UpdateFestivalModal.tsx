...
interface UpdateFestivalModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSave: (updatedData: { title: string; description: string; rules: string[]; startDate: string; endDate: string }) => void;
}

export function UpdateFestivalModal({ festival, isOpen, onClose, onSave }: UpdateFestivalModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      startDate: formData.startDate ? new Date(formData.startDate).toISOString() : festival.startDate,
      endDate: formData.endDate ? new Date(formData.endDate).toISOString() : festival.endDate,
    });
    onClose();
  };
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              placeholder="E.g. The Autumn Rite"
            />
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea
              ...
              onChange={(e) => setFormData((p) => ({ ...p, rulesText: e.target.value }))}
              className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm font-medium text-white focus:border-white focus:ring-1 focus:ring-white/20 outline-none transition-all resize-none placeholder:text-white/10"
