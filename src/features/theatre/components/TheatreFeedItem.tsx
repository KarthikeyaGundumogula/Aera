...
export const TheatreFeedItem = memo(function TheatreFeedItem({
  ...
  return (
    ...
    >
      {/* Media Container */}
      <div
        className={`relative rounded-xl overflow-hidden bg-white/5 border mb-3 ${isPoster ? "border-transparent group-hover:border-white/10 ring-1 ring-white/5" : "border-white/5"}`}
      >
        {isScript ? (
          <div className="w-full min-h-[300px] bg-[#f4f1ea] text-[#2a2a2a] p-6 md:p-8 font-mono text-[10px] md:text-xs leading-tight overflow-hidden shadow-inner border border-black/5 flex flex-col justify-center select-text transition-transform duration-700 group-hover:scale-[1.02]">
            <div className="uppercase mb-2 opacity-40 text-[7px] md:text-[8px] font-bold tracking-widest">
              Scene {item.id}
            </div>
            ...
            <div className="mb-4 italic opacity-70 leading-relaxed text-sm md:text-base">
              {item.title?.split(":").length! > 1
                ? item.title!.split(":")[1]
                : item.text || "A moment of pure cinematic reflection."}
            </div>
            <div className="text-center w-full mb-1 mt-2 font-bold uppercase text-[8px] md:text-[10px] tracking-[0.2em]">
