interface ThoughtContextTag {
  type: "Original" | "Work" | "Artist";
  id: string;
  title: string;
  image?: string;
}

export interface ThoughtEmbeddedWork {
  id: string;
  title: string;
  category?: string;
  image?: string;
  artistName?: string;
}

export interface ThoughtItem {
  id: string;
  title?: string;
  content?: string;
  authorId: string;
  authorName: string;
  authorAvatar?: string;
  text: string; // Deprecated alias for content / message
  hits: number;
  threadCount?: number;
  reactionCount?: number;
  topReactions?: ("heart" | "zap" | "flame" | "star" | "sparkles")[];
  timestamp: string;
  tag?: ThoughtContextTag;
  work?: ThoughtEmbeddedWork;
  setId?: string;
  setName?: string;
}

export const THOUGHTS_MOCK: ThoughtItem[] = [
  {
    id: "th-1",
    title: "Rhythmic Pacing of OG's Second Act",
    content: "The pacing in the second act of OG isn't just fast; it's rhythmic ☀️. Sujeeth treats every cut like a beat in a metronome. You don't watch it, you feel the tempo.",
    text: "The pacing in the second act of OG isn't just fast; it's rhythmic ☀️. Sujeeth treats every cut like a beat in a metronome. You don't watch it, you feel the tempo.",
    authorId: "art-1",
    authorName: "KAIF",
    authorAvatar: "https://i.pinimg.com/1200x/c0/83/8d/c0838da1580980cf7bcda233de7361bf.jpg",
    hits: 84,
    threadCount: 12,
    reactionCount: 342,
    topReactions: ["heart", "flame", "zap"],
    timestamp: "2 hours ago",
    setId: "set-lcu",
    setName: "LCU Syndicates",
    work: {
      id: "w-og-1",
      title: "OG Intro Blast",
      category: "Edit",
      image: "https://img.youtube.com/vi/GG1_DsScm6U/hqdefault.jpg",
      artistName: "PowerStar_FC",
    },
    tag: {
      type: "Original",
      id: "og-original",
      title: "OG",
      image: "https://i.pinimg.com/1200x/ab/f6/8b/abf68b35ce82d8c97dd3d100c8227b40.jpg",
    },
  },
  {
    id: "th-2",
    title: "Visual Warmth & Color Mastery in RRR",
    content: "RRR’s water and fire motif isn't subtle, but the color grading in the pre-interval scene physically raises the temperature of the screen. Absolute mastery of visual warmth.",
    text: "RRR’s water and fire motif isn't subtle, but the color grading in the pre-interval scene physically raises the temperature of the screen. Absolute mastery of visual warmth.",
    authorId: "art-2",
    authorName: "NEXUS",
    authorAvatar: "https://i.pinimg.com/736x/44/d5/47/44d547fecab1aeb9bb359677353f81e3.jpg",
    hits: 120,
    threadCount: 45,
    reactionCount: 892,
    topReactions: ["flame", "sparkles", "star"],
    timestamp: "5 hours ago",
    setId: "set-baahubali",
    setName: "Mahishmati Lore",
    work: {
      id: "w-rrr-1",
      title: "Naatu Naatu Hype",
      category: "Edit",
      image: "https://img.youtube.com/vi/D9O20qxyvT0/hqdefault.jpg",
      artistName: "Ram_Charan_FC",
    },
    tag: {
      type: "Original",
      id: "rrr-original",
      title: "RRR",
      image: "https://i.pinimg.com/1200x/e0/8a/a5/e08aa58ed4dc0c3ceccfe8dce28ff0b8.jpg",
    },
  },
  {
    id: "th-3",
    title: "Claustrophobia & Negative Space in KGF",
    content: "Why does no one talk about the negative space in KGF's underground scenes? The darkness isn't just lighting; it's a character pressing down on Rocky. Claustrophobia as an art form.",
    text: "Why does no one talk about the negative space in KGF's underground scenes? The darkness isn't just lighting; it's a character pressing down on Rocky. Claustrophobia as an art form.",
    authorId: "art-3",
    authorName: "VALERA",
    authorAvatar: "https://i.pinimg.com/736x/2b/bb/68/2bbb68f94cb4c55ecb121b6191b2c6a0.jpg",
    hits: 42,
    threadCount: 8,
    reactionCount: 156,
    topReactions: ["zap", "heart"],
    timestamp: "1 day ago",
    setId: "set-aot",
    setName: "Titan Vanguard",
    work: {
      id: "w-kgf-1",
      title: "Rocky Bhai Entry",
      category: "Edit",
      image: "https://img.youtube.com/vi/us9PTfKlvuY/hqdefault.jpg",
      artistName: "KGF_Yash_FC",
    },
    tag: {
      type: "Original",
      id: "kgf-original",
      title: "KGF",
      image: "https://i.pinimg.com/736x/cd/6d/46/cd6d4608ed1ba763b652886f4a56d98f.jpg",
    },
  },
  {
    id: "th-4",
    title: "Frame Geometry & Character Alignment in Pushpa",
    content: "Pushpa doesn't just walk; he commands the geometry of the frame. Sukumar stages him so that every other character subconsciously aligns to his posture.",
    text: "Pushpa doesn't just walk; he commands the geometry of the frame. Sukumar stages him so that every other character subconsciously aligns to his posture.",
    authorId: "art-4",
    authorName: "VORTEX",
    authorAvatar: "https://i.pinimg.com/736x/11/1d/15/111d152a5dbfe972e2cf1cb58941655d.jpg",
    hits: 95,
    threadCount: 23,
    reactionCount: 430,
    topReactions: ["star", "flame", "sparkles"],
    timestamp: "2 days ago",
    setId: "set-bb",
    setName: "Blue Sky Chemists",
    tag: {
      type: "Original",
      id: "pushpa-01",
      title: "PUSHPA",
      image: "https://i.pinimg.com/736x/88/2c/31/882c31c072b2a6f23c9ce0550cf6bbdb.jpg",
    },
  },
];
